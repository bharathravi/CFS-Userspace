...
extern void uthread_schedule(uthread_struct_t * (*kthread_best_sched_uthread)(kthread_runqueue_t *))
{
  ...
  if((u_obj = kthread_runq->cur_uthread)) {
    // If there is a thread already running, pre-empt it, account its vruntime and
    // add it back to the runqueue
    // TODO: Instead of repeatedly removing and adding, try to optimise
    kthread_runq->cur_uthread = NULL;
    if(u_obj->uthread_state & (UTHREAD_DONE | UTHREAD_CANCELLED)) {
      // If this thread has finished executing, throw it away.
...
//      gt_spin_unlock(&(kthread_runq->kthread_runqlock));
      ...
      //XXX(CFS) This thread has voluntarily given up CPU. 
      // We must ensure it does not run again in this period. So, we set 
      // its vruntime to the maximum vruntime +1, so that it is inserted in
      // the rightmost end of the RB Runqueue.
...
//      printf("Thread %d has run for %d\n", u_obj->uthread_tid, u_obj->vruntime);
      u_obj->uthread_state = UTHREAD_RUNNABLE;
      add_to_runqueue(kthread_runq->runqueue, &(kthread_runq->kthread_runqlock), u_obj);
  ...
  if(!(u_obj = kthread_best_sched_uthread(kthread_runq)))
  {
    ...
    {
     // XXX(CFS): This if condition says "if there was atleast one thread ever created, and now there are none
     // then exit. This means it will not exit immediately after the app is started, before even the first
     // uthread is run.
  ...
  // Take this thread out of the runqueue and set its state as running
  rem_from_runqueue(kthread_runq->runqueue, &(kthread_runq->kthread_runqlock), u_obj);
//  printf("Starting thread %d with vruntime %d\n", u_obj->uthread_tid, u_obj->vruntime);
  u_obj->uthread_state = UTHREAD_RUNNING;
  // Record when the thread entered CPU
  gettimeofday(&now, 0);
...
static inline long get_fair_slice(uthread_struct_t* uthread, cfs_runqueue_t * runqueue) {
  ...
  int num_threads = runqueue->num_threads;
...
extern int uthread_create(uthread_t *u_tid, int (*u_func)(void *), void *u_arg, uthread_group_t u_gid)
{
  ...
  //TODO Calculate appropriate vruntime for this thread and add it to the runqueue
  u_new->vruntime = 0;
