...
static void kthread_init(kthread_context_t *k_ctx)
{
  ...
  // Yield so that I can be scheduled on the CPU to which I have affinity
  sched_yield();
...
extern kthread_runqueue_t *ksched_find_target(uthread_struct_t *u_obj)
{
  ...
  do
  {
    ...
    target_cpu = ((target_cpu + 1) % GT_MAX_CORES);
        ...
        //printf("Target CPU for %d is: %d\n", u_obj->uthread_tid, target_cpu);
...
static void gtthread_app_start(void *arg)
{
  ...
  {
    //printf("Kthread %d, looking for a thread to schedule\n", kthread_apic_id()); 
...
extern void gtthread_app_init()
{
  ...
  //kthread_install_sighandler(SIGUSR1, k_ctx_main->kthread_sched_relay); // No relay scheduling
...
extern void gtthread_app_exit()
{
  ...
  while(!(k_ctx->kthread_flags & KTHREAD_DONE))
  {
    //printf("Wrapping up main tasks\n");
    // XXX(CFS): In the original implementation of gtthreads, this loop
    // goes on infinitely if the app has not created a user thread. For example, if I
    // have a main function that simply calls "app_init" and "app_exit", it will never
    // finish. I believe this to be a bug, but am not fixing this right now.
    __asm__ __volatile__ ("pause\n");
    if(sigsetjmp(k_ctx->kthread_env, 0))
    {
      /* siglongjmp to this point is done when there
