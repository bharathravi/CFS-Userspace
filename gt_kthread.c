...
// XXX(CFS): Fixing bug in gtthreads. This master thread is the thread of the "main"
// function that the user calls.
static void create_master_uthread(kthread_context_t* master_context, sigjmp_buf master_stack) {
	kthread_runqueue_t *kthread_runq;
        ...
        u_new->uthread_state = UTHREAD_RUNNABLE;
        u_new->uthread_gid = 0;
        u_new->uthread_func = NULL;
